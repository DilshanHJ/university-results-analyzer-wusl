...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    api.post<{ accessToken: string; user: AuthUser }>('/auth/refresh').then(({ data }) => {
  ...
  const login = useCallback(async (identifier: string, password: string) => {
    const { data } = await api.post<{ accessToken: string; user: AuthUser }>('/auth/login', { identifier, password });
    setAccessToken(data.accessToken);
    setUser(data.user);
  }, []);
